...
generator \
  ...
  -exclude_state \
  -schema_struct_path=github.com/stolsma/go-p4dpdk-vswitch/pkg/gnmi/internal/config \
  -output_dir=config \
  -package_name=device \
  -path_structs_split_files_count=5 \
  -split_pathstructs_by_module=true \
  -path_structs_output_file=config/device/device.go \
  -base_import_path=github.com/stolsma/go-p4dpdk-vswitch/pkg/gnmi/internal/config \
...
generator \
  ...
  -split_pathstructs_by_module=true \
  -schema_struct_path=github.com/stolsma/go-p4dpdk-vswitch/pkg/gnmi/internal/telemetry \
  -path_struct_package_suffix="" \
  -base_import_path=github.com/stolsma/go-p4dpdk-vswitch/pkg/gnmi/internal/telemetry \
  -path_structs_split_files_count=10 \
  "${COMMON_ARGS[@]}" \
  "${YANG_FILES[@]}"
